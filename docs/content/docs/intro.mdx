...
On app startup, you can restore the user's session from the stored token using `hydrate()`. It returns the session if a valid token exists, or `null` otherwise.

```dart
final session = await client.hydrate();
if (session != null) {
