...
class Signup {
  ...
  Signup({required Dio dio, required Function(String) setToken, User Function(Map<String, dynamic>)? fromJsonUser}) : _dio = dio, _setToken = setToken, _fromJsonUser = fromJsonUser;

  Future<BetterAuthClientResponse<User, Exception>> email(String email, String password, String name, {String? image}) async {
    try {
      final response = await _dio.post("/sign-up/email", data: {"email": email, "password": password, "name": name, "image": image});
